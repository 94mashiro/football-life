...
function DecisionBoard({ choices, purist, showHint, onPick }: {
  choices: readonly Choice[]; purist: boolean; showHint: boolean; onPick: (id: string) => void;
}) {
          ...
          <button key={c.id} className="option" onClick={() => onPick(c.id)}>
            <span className="option-lead">
              ...
              <span className="font-semibold">
                ...
                {c.trophyOdds && <TrophyOddsRow odds={c.trophyOdds} purist={purist} />}
                {showHint && i === 0 && <span className="hint-badge ml-2 align-middle">推荐</span>}
              </span>
            </span>
  ...
  const fate = offers.length === 1 && baseline.length === 0;
  return (
    <div className="deck-options">
          ...
          {outcomeFor && game.lastOutcome ? (
            ...
            <div className="dock-decision anim-slide">
              ...
              <DecisionBoard choices={game.pendingChoice.choices} purist={!!purist}
                showHint={game.seasons.length < 3} onPick={pick} />
            </div>
          ) : (
