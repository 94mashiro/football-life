...
export function resolveEventOption(
  ...
  switch (`${key}:${optionKey}`) {
    ...
    case "signature_skill:master_it": {
      ...
      good = true;
      outcome = success
        ? "你继续了。你的任意球成了联赛里最 feared 的武器——门将看到你站在球前就开始紧张。你进了77个任意球，比历史上任何人都多。你退役后人们不再记得你的跑动、你的防守、你的传球。他们只记得你的任意球——那把像Excalibur一样的武器。你用一生练一技。够了。"
        : "你继续了。你的任意球确实很厉害——但对手开始研究了。他们犯规时不再在你附近犯规了。你的任意球机会少了，你的武器被对手的战术封印了。也许你需要一个Plan B。也许你只需要等待他们犯错。你等着。";
      break;
    }
...
export function loanOfferEvent(ctx: EventContext): FiredEvent {
  ...
  return {
    event: { key: "loan_offer", title: "租借邀约", desc: "出场时间有限，是否租借练级？", choices },
    resolve: (choice) => {
      if (choice.id === "stay") {
        return { mods: { loyalStay: true }, outcome: `你留在 ${contractClub.name}。`, good: true };
      }
      const idx = Number(choice.id.replace("loan-", ""));
      const offer = offers[idx];
      if (!offer) return { mods: {}, outcome: "未达成租借。", good: false };
      return { mods: { loanOutTo: offer.club.id, loanReturnAge: returnAge }, outcome: `你租借至 ${offer.club.name}。`, good: true };
    },
  };
}
...
export function blockbusterOfferEvent(ctx: EventContext, maxOverall: number, offeredTier: number | undefined): FiredEvent | null {
  ...
  const choices: Choice[] = [
    { id: `join-${pick.id}`, kind: "new_club", text: `加盟 ${pick.name}`, sub: `${LEAGUES.find((l) => l.id === pick.leagueId)?.name ?? ""} · ${"★".repeat(pick.rep + 1)}` },
    { id: "stay", kind: "stay", text: `留在 ${currentClub.name}` },
  ];
  return {
    event: {
      key: "blockbuster_offer", title: "豪门邀约",
      desc: `${pick.name} 向你抛来橄榄枝——这是职业生涯的巅峰转会。`,
