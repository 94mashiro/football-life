...
export function optionOdds(key: string, ctx: EventContext): number | undefined {
  switch (key) {
    ...
    // P-DEGEN: doctor_warning:defy is now a 50/50 gamble (keep the aggressive
    // edge vs the body breaks) — mirrors the inline roll in resolveEventOption.
    case "doctor_warning": return 0.5;                   // defy: 无视医嘱硬刚
    // P-DEGEN: father_agent:assert_independence rolls 0.5 — surface it so the
    // pill shows both branches instead of a forced-positive lie.
    case "father_agent": return 0.5;                    // assert_independence: 自立赌一把
...
export function resolveEventOption(
  ...
  switch (`${key}:${optionKey}`) {
    ...
    case "doctor_warning:defy": {
      ...
      // compromised_body@6（身体替你做了决定）。与 optionOdds["doctor_warning"]=0.5 同步。
    ...
    case "father_agent:trust_father":
      // P-DEGEN: 「听父亲的」曾是零回报陷阱（且 optionOdds 没显 assert 的 50%，令其谎报为确定 +1）。
      // 现为安全路：稳涨 +1 perm（父亲选的俱乐部是好平台），代价是你不是自己的人 →
      // roleShift −1（俱乐部把你当你父亲的项目，不是独立的领袖）。
      // 隔壁 assert_independence 现走 optionOdds=0.5 显双分支（PROB_OPTION_KEYS 已加 assert_independence）。
      mods.permanentOverallDelta = 1; mods.roleShift = -1;
      good = true;
...
const PROB_OPTION_KEYS = new Set([
  ...
  "prove_yourself",
  "fight_for_spot",
  // P-DEGEN: father_agent:assert_independence rolls 0.5 — without this it was
  // shown as a lying certain "+1 OVR" pill (forced-positive preview of a 50/50
  // gamble). Surfacing both branches is honest.
  "assert_independence",
]);
