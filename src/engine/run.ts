...
export function simulatePeriod(state: GameState): GameState {
  ...
  for (let i = 0; i < periodLength; i++) {
    ...
    // 0 出场/伤病季(rating null)记 0 分 —— 宽限, 与 forced-exit 的 grace 一致。
    const sr = season.rating;
    const ratingScore = sr == null ? 0
      : (RATING_GROWTH_BANDS.find((b) => sr - forcedExitBar(club) >= b.minDiff)?.delta ?? 0);
    let delta = growthDelta(rng, player, season.role, club, state.ascension, declineDelay, ratingScore);
