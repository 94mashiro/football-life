 ...
 */
import { drive, POLICIES, digest, copyDigest, canonical, corpusSeed } from "./_harness";
...
console.log(`${key}  seed=${seed}  行为=${digest(t)}  文案=${copyDigest(t)}`);
...
if (process.env.COPY) {
  console.log("\n文案 (事件卡面 / 选项 / 结算 / 故事线):");
  for (const c of t.copy) console.log(`  ${c}`);
}
