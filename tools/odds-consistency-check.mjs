...
// ── optionOdds：解析每个 `case "k:o":` → 返回表达式（字面量或计算式）。
//   三种形：同行 case+return / 跨行 return / fall-through 多 case 共用一 return。
const [ooStart, ooEnd] = fnRange("optionOdds");
const ooCases = new Map();
let pending = [];
const ooAssign = (expr) => {
  const lit = litNum(expr);
  for (const k of pending) ooCases.set(k, { literal: lit, expr });
  pending = [];
};
for (let i = ooStart; i < ooEnd; i++) {
  const line = src[i];
  if (/^\s*default:/i.test(line)) { pending = []; continue; }
  // 同行 case + return：case "k:o": return expr;
  const same = line.match(/^\s*case "([a-z0-9_]+:[a-z0-9_]+)":\s*return (.+?);\s*$/);
  if (same) { pending.push(same[1]); ooAssign(same[2]); continue; }
  const cm = line.match(/^\s*case "([a-z0-9_]+:[a-z0-9_]+)":/i);
  if (cm) { pending.push(cm[1]); continue; }
  const rm = line.match(/^\s*return (.+?);\s*$/);
  if (rm && pending.length > 0) ooAssign(rm[1]);
}
