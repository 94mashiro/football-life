...
import { applyCeiling, devRep } from "../src/engine/sim";
...
for (const [id, label] of cases) {
  console.log(`${label.padEnd(10)} ${clubById(id).name.padEnd(8)} eff-rep ${devRep(clubById(id))} → 硬顶 ${hardTop(id)}`);
}
...
console.assert(devRep(clubById("beijing-guoan")) < clubById("beijing-guoan").rep, "中超应有 −1 档偏移");
