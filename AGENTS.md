...
**`buildPeriodDecision` priority**: medical emergencies (verdict/warning) → post-loan → contract/relegation contextual → climax events (WC ages, OVR-gated) → `decisivePenalty` (21/25, starter ≥75) → retention gate (age ≥33) → injury → **transfer window** (age-based cadence: every 2 seasons through the prime years 19–31 → ~7 windows/career, pace-independent; 飞升 8 → every 5) → career-plan slots → throne/blockbuster/loan → random event → silent advance. **Transfers are the career spine** (参考 Copero: 转会窗为最常见决策, 生涯 ~7 家俱乐部): the cadence is age-based (pace-independent) and capped at the prime years — the late career is left to the decline/retirement arc + silent periods. The career-plan slots sit **below** the transfer window (a due window fires first; the plan fills the non-window periods) — hoisting the plan *above* the window let it eat windows that, at the default pace (window due every period), the `transferWindowOwed` rollover then *merged* with the next due window (a net loss, not a defer), starving transfers to ~1.5/career. A due window that a peak/medical event overrides **rolls over to next period** (`transferWindowOwed`) rather than being cancelled, so events defer transfers, never starve them. Don't remove the cadence — players need it to climb to bigger clubs.
