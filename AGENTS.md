...
- **Pointer Events for gestures, never `onTouch*`/`onMouse*`**: drag/swipe/tap-tracking handlers use `onPointerDown/Move/Up/Cancel` (+ `setPointerCapture`, `e.pointerType`) so mouse, touch, and pen share one path. `onClick` on a `<button>` already synthesizes exactly once on tap — no double-fire, no 300ms delay (the viewport meta killed that years ago). Enforced by `npm run lint:touch` and the pre-commit hook. The mobile interaction baseline (transparent `-webkit-tap-highlight-color`, `-webkit-touch-callout: none`, `user-select: none`, `touch-action: manipulation` on interactive controls; opt-outs via class specificity like `.share-img`) lives in `index.css` `@layer components`.
